...
export default function StatsBox() {
    ...
    async function apiOptionsCall(id, options, value, operator) {
        ...
        await api.getAxiosEndpoint(endpoints.API_LEAGUES + '/' + id + '/stats/' + options)
        .then((response) => {
    ...
    const cardStats = () => {
        return (
            <>
                <ul>
                    <li className={showPlayers === true ? "color-selected left line mb5" : "left line mb5"} onClick={() => handleClickOptions(statsTypes.PLAYERS, showPlayers, setShowPlayers)}>
